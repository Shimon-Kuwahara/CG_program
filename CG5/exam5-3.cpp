...
void display(void) {        
  ...
  // 接戦を描画
        ...
        // 接戦の長さを曲率によって決定
        BezierCurvature = 6 * (1-t) * g_ControlPoints[i-3] + (18 * t - 12)* g_ControlPoints[i-2] + (-18 * t + 6) * g_ControlPoints[i-1] + 6 * t * g_ControlPoints[i];

				curvature = (BezierNormal.x * BezierCurvature.y - BezierCurvature.x * BezierNormal.y) / pow((pow(BezierNormal.x, 2.0) + pow(BezierNormal.y, 2.0)), 3.0 / 2.0);

				BezierNormal = (BezierNormal / BezierNormal.length()) * 3000 * curvature;
