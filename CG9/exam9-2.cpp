...
public:
	...
	Vector3d getColorVec(double x, double z) {
		...
		if(x > 0){
			if (((static_cast<int>(x) / 30) + (static_cast<int>(z) / 30)) % 2 == 0) {
					return Vector3d(0.8, 0.6, 0.6);
			} else {
					return Vector3d(0.7, 1.0, 1.0);
			}
		}
		else{
			if (((static_cast<int>(x) / 30) + (static_cast<int>(z) / 30)) % 2 == 0) {
...
void getPixelColor(double x, double y, Vector3d &colorVec) {
	...
	if(t_sphere > 0) { // 球との交点がある
		...
		Vector3d rayReflect = ray + 2 * N;
		Vector3d boardColor;
		rayReflect.normalize();
		double t_board = board.getIntersec(P, rayReflect);
		boardColor = P + t_board * rayReflect;
		if (t_board > 0)
		{
			boardColor = board.getColorVec(boardColor.x, boardColor.z);
			// r,g,b = (玉の色 + 床の色) / 2;
			double r = (std::min(I * sphere.cR, 1.0) + std::min(I * boardColor.x, 1.0))/2.0;
			double g = (std::min(I * sphere.cG, 1.0) + std::min(I * boardColor.y, 1.0))/2.0;
			double b = (std::min(I * sphere.cB, 1.0) + std::min(I * boardColor.z, 1.0))/2.0;
			colorVec.set(r, g, b);
		}
		else
		{
	...
	// 何とも交差しない
	colorVec.set(220, 220, 220); // 背景色（黒）を設定する
}
